...
function EventForm({ userId, type, event, eventId }: EventFormProps) {
  ...
  // Define de initial Values if the form is 'update' or if not
  const initialValues = event && type === 'Update' ? { ...event, startDateTime: new Date(event.startDateTime), endDateTime: new Date(event.endDateTime), categoryId: event.category._id } : eventDefaultValues;

  // The method to upload images and the form schema
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-5">
            ...
            render={({ field }) => (
              <FormItem className="w-full">
                <FormControl className="h-72">
                  <Textarea placeholder="Description" className="textarea rounded-2xl"
                    {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
              ...
              <FormItem className="w-full">
                <FormControl>
                  <div className="flex-center h-[54px] w-full overflow-hidden rounded-full bg-grey-50 px-4 py-2">
                    ...
                    <DatePicker selected={field.value} onChange={(date: Date) => field.onChange(date)} showTimeSelect timeInputLabel="Time:" dateFormat="MM/dd/yyyy h:mm aa"
              ...
              <FormItem className="w-full">
                <FormControl>
                  <div className="flex-center h-[54px] w-full overflow-hidden rounded-full bg-grey-50 px-4 py-2">
                    ...
                    <Input placeholder="Price" type="number" className="input-field" {...field} />
        ...
        <Button type="submit" size="lg" disabled={form.formState.isSubmitting} className="col-span-2 button w-full">
